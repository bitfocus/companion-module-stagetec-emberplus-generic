...
class EmberPlusInstance extends InstanceBase<EmberPlusConfig> {
  ...
  public async destroy(): Promise<void> {
    this.emberClient.disconnectAsync()
  }
  ...
  private setupParseFilters(): void {
    ...
    if (this.config.autoParsePathsString)
      this.config.autoParsePaths = this.config.autoParsePathsString.split(',').map((path) => <parsingPath>{path: path, elements: path.split('.')})
  ...
  private async registerParameters() {
    ...
    for (const param of this.config.parseParameterPaths ?? [])
    {
      try 
      {
        let path_nodes = param.id.split('.')
        let param_num = path_nodes[path_nodes.length - 1]
        path_nodes.pop()

        let parent_node : TreeNode = await this.emberClient.getElementByPathAsync(path_nodes.join('.'))

        let param_node = parent_node.getElementByNumber(Number(param_num)) as TreeNode
  ...
  private async handleChangedValue(path: string, node: TreeNode) {
    if (node.isParameter()){
      ...
      for (const feedback in FeedbackId)
  ...
  private async handleEmberTreeParsing(node: TreeNode, identifiers: string, curr_layer: number)
  {
    ...
    if ((node.isRoot() || node.isNode()) && this.config.monitoredParameters.length < 2048)
    {
      if (node.hasChildren()){
        ...
        {
          let curr_child = child as TreeNode
          let identifier = curr_child.getJSONContent()['identifier']?.replace('#','')

          // check if node is online
          if (curr_child.isNode() && !curr_child.getJSONContent()['isOnline'])
            continue
          ...
          if (this.config.autoParsePaths && this.config.autoParsePaths.length > 0)
          {
            ...
            {
              if ((curr_layer >= entry.elements.length && curr_child.getJSONContent()['path'].startsWith(entry.path)) || (curr_layer < entry.elements.length && entry.path.startsWith(curr_child.getJSONContent()['path'])))
              {
                if (curr_child.isNode())
                  await this.emberClient.getDirectoryAsync(curr_child);

                if (identifiers == '')
                  await this.handleEmberTreeParsing(curr_child, identifier || '', curr_layer + 1);
                else
                  await this.handleEmberTreeParsing(curr_child, identifiers + '.' + identifier || '', curr_layer + 1);
                break;
              }
            }
          }
            ...
            if (identifiers.includes(nodeFilter))
            {
              if (this.config.parseParamFilter?.length)
              {
                for (const paramFilter of this.config.parseParamFilter ?? []) 
                {
                  ...
                  {
                    this._addMonitoredParameter(node, identifiers)
                    return
                  }   
                }
              }
